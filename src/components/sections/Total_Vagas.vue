<template>
    <div v-if="hasValueInMap" id="total_vagas" class="section-class"
        :class="{ 'bg-gray-800 text-white': map.total_vagas.posclick }">
        <h1 class="font-semibold text-lg mb-1">Total_vagas:</h1>
        <p v-if="map.total_vagas.comentario">{{ map.total_vagas.comentario }}</p>
        <span v-if="map.total_vagas.xpath">{{ map.total_vagas.xpath }}</span>
        <button v-if="map.total_vagas.xpath" class="button-custom" data-type="xpath"
            @click="emitCopyContent(map.total_vagas.xpath, $event.target)">
            XPath
        </button>
        <div v-for="processador in processadoresMapped.total_vagas" :key="processador.nome">
